...
const WordEntry: React.FC<Iprops> = (props: Iprops) => {
  ...
  const handleClick = (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
    e.preventDefault();
    console.log(
      `Asking for puzzle with ${words.map((word) => word.word).join(", ")}`
    );
    refetch({
      url:
        ...
        <Form.Item
          ...
          rules={[
            ...
            },
            {
              ...
              message: "Only A-Z or spaces allowed",
            },
            {
              validator: (rule, value) =>
                words
                  .map((wordInList) => wordInList.word)
                  .includes(value.trim().toUpperCase())
                  ? Promise.reject(
                      `${value.trim().toUpperCase()} is already included!`
                    )
                  : Promise.resolve(),
            },
          ]}
